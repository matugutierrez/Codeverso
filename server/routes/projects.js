...
<style>*{box-sizing:border-box}body{margin:0;min-height:100vh;background:#090d16;color:#edf2ff;font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif}.wrap{min-height:100vh;display:grid;grid-template-columns:minmax(320px,430px) 1fr}.panel{padding:34px;background:#111827;border-right:1px solid #273244}.kicker{color:#93c5fd;font-size:12px;text-transform:uppercase;letter-spacing:.08em;font-weight:800}.panel h1{font-size:32px;line-height:1.05;margin:10px 0 12px}.panel p{color:#cbd5e1;line-height:1.5}.cmds{display:flex;flex-direction:column;gap:10px;margin:22px 0}.cmd{display:grid;grid-template-columns:28px 1fr;gap:10px;align-items:center;background:#030712;border:1px solid #334155;border-radius:8px;padding:10px}.cmd small{width:24px;height:24px;border-radius:50%;display:grid;place-items:center;background:#2563eb;color:#fff;font-weight:800}.cmd code{white-space:pre-wrap;word-break:break-word;color:#86efac;font:13px/1.35 'Share Tech Mono',Consolas,monospace}.url{display:flex;gap:8px;margin-top:16px}.url input{min-width:0;flex:1;border:1px solid #334155;background:#020617;color:#fff;border-radius:7px;padding:11px}.url button,.copy{border:1px solid #2563eb;background:#2563eb;color:white;border-radius:7px;padding:11px 13px;font-weight:800;cursor:pointer}.copy{width:100%;margin-top:8px;background:#1f2937;border-color:#475569}.stage{background:#020617;min-height:100vh;display:grid;place-items:center;padding:18px}.empty{border:1px dashed #334155;border-radius:12px;width:min(720px,100%);padding:34px;text-align:center;color:#94a3b8}.live{width:100%;height:100vh;border:0;background:white;display:none}.badge{position:fixed;right:10px;bottom:10px;background:#020617;color:#86efac;border-radius:5px;padding:6px 9px;font:11px 'Share Tech Mono',Consolas,monospace}@media(max-width:850px){.wrap{grid-template-columns:1fr}.stage{min-height:420px}.live{height:70vh}}</style></head><body><div class="wrap"><aside class="panel"><div class="kicker">Preview local requerido</div><h1>${escapeHtml(p.name)}</h1><p>No se puede renderizar esta app fullstack como HTML suelto: necesita dependencias, build y servidor. Ejecuta estos comandos en tu maquina y despues abre la URL local aca.</p><div class="cmds">${commandBlocks}</div><button class="copy" type="button" id="copy">Copiar comandos</button><div class="url"><input id="url" value="${escapeHtml(plan.localUrl)}" spellcheck="false"><button id="open" type="button">Abrir</button></div></aside><main class="stage"><iframe class="live" id="live" title="Preview local"></iframe><div class="empty" id="empty"><h2>Esperando servidor local</h2><p>Cuando ejecutes los comandos, pega la URL local y se vera en este recuadro.</p></div></main></div><div class="badge">preview local</div><script>var cmds=${JSON.stringify(plan.commands.join('\n'))};document.getElementById('copy').onclick=function(){navigator.clipboard&&navigator.clipboard.writeText(cmds)};document.getElementById('open').onclick=function(){var u=document.getElementById('url').value.trim();if(!u)return;document.getElementById('empty').style.display='none';var f=document.getElementById('live');f.style.display='block';f.src=u};</script></body></html>`
}
